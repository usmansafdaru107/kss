...
function logout()
{
	...
	if(isset($_SESSION['user']) && !empty($_SESSION['user']) && isset($_SESSION['track']) && !empty($_SESSION['track'])){
		$db=connect_db();
		$logout=$db->prepare("UPDATE logins SET status=0 WHERE login_id=? AND session_id=?");
		$logout->bind_param('is',$login,$session);
		$login=$_SESSION['track'];
		$session=session_id();
		if(!$logout->execute()){
			die($db->error);
		}else{
			...
			if(!$fetch_admin)
				{
					...
					{
						echo json_encode($fetch_admin->fetch_all(MYSQLI_ASSOC));
					}
				$db->close();
	}
	else
...
function delete_admin($id)
{
	if(check_admin())
	{
		...
		if(!$update_db){
			...
			$db->close();
			echo json_encode(array('status'=>'Success','message'=>'Tutor deleted successsully!!'));
			exit();
		}
	}
...
function check_admin(){
	...
	if(isset($_SESSION['user']) && !empty($_SESSION['user']) && isset($_SESSION['status']) && $_SESSION['status']===true && isset($_SESSION['account']) && strcmp($_SESSION['account'],'admin')===0)
	{
		return true;
	}
	else{
...
function check_student(){
	...
	if(isset($_SESSION['user']) && !empty($_SESSION['user']) && isset($_SESSION['status']) && $_SESSION['status']===true && isset($_SESSION['account']) && strcmp($_SESSION['account'],'student')===0)
	{
		return true;
	}
	else{
...
function check_sponsor(){
	...
	if(isset($_SESSION['user']) && !empty($_SESSION['user']) && isset($_SESSION['status']) && $_SESSION['status']===true && isset($_SESSION['account']) && strcmp($_SESSION['account'],'sponsor')===0)
	{
		return true;
	}
	else{
...
function reset_admin_password($admin_id){
