	...
	else if(isset($quiz) && !empty($quiz)) 
	{
		...
		else
		{
			if($fetch_quiz->num_rows>0)
			{
				...
				{
					$quiz_arr['questions']=$fetch_qns->fetch_all(MYSQL_ASSOC);
				}
...
function set_up_qn($id)
{
	...
	if(isset($id) && !empty($id))
	{
		...
		else
		{
				...
				$qn_arr=array();
				$qn_arr['instruction']=$instruction;
				$qn_arr['instruction']=$res['instruction_id'];
				if($res['qn_type']==1)
				{
					$qn_tpe='objective';
				}
				...
				{
					$qns=$qn_sel->fetch_all(MYSQL_ASSOC);
					...
					{	
						...
						if(!$qns_sel)
						{
							...
							{
								$qn['answers']=$qns_sel->fetch_all(MYSQL_ASSOC);
							}
							...
							{
								$qn_arr['resources']=$res_sql->fetch_all(MYSQL_ASSOC);
							}
	...
	if(isset($question) && !empty($question) && is_string($question))
		{
			if($qns=json_decode($question))
			{
				...
				{
					$qn_query->bind_param('qi',$qn_str,$instr);
					...
					if(!$qn_query->execute())
						{
							...
							{
								$qn_id=$db->insert_id;
								add_answer($qn_id,$qn_base->answers,$db);
