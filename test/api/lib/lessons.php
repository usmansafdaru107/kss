	...
	function fetch_lessons($id='',$topic='')
	{
		...
		else if(!empty($topic))
		{
			if($pagination==true)
				...
				{
					$sql=$db->query("SELECT l.lesson_id,t.topic_id,l.name,t.topic_name FROM lessons AS l JOIN topics AS t ON t.topic_id=l.topic WHERE t.topic_id='$topic' AND t.status='1' AND l.status='1' ORDER BY l.lesson_id ASC LIMIT $start,$pageSize");
				}
		...
		else
		{
			if($pagination==true)
				...
				{
					$sql=$db->query("SELECT l.lesson_id,t.topic_id,l.name,t.topic_name FROM lessons AS l JOIN topics AS t ON t.topic_id=l.topic WHERE t.status='1' AND l.status='1' ORDER BY l.lesson_id ASC LIMIT $start,$pageSize");
				}
